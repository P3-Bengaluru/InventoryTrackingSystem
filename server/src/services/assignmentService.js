...
async function getAll(filters = {}) {
  ...
  if (filters.status) query.andWhere({ 'assignments.status': filters.status });
  if (filters.asset_id) query.andWhere({ 'assignments.asset_id': filters.asset_id });
  if (filters.user_id) query.andWhere({ 'assignments.user_id': filters.user_id });
...
async function create(data, req) {
  ...
  if (asset.status === 'assigned') throw new AppError('Asset is already assigned', 400);
  if (asset.status === 'maintenance') throw new AppError('Asset is under maintenance', 400);
  if (asset.status === 'retired' || asset.status === 'disposed') throw new AppError('Asset is not available', 400);

  const user = await db('users').where({ id: data.user_id, is_active: true }).first();
  ...
  const admins = await db('users').whereIn('role', ['admin', 'inventory_manager']).andWhere({ is_active: true }).select('id');
...
async function approve(id, req) {
  ...
  if (!assignment) throw new AppError('Assignment not found', 404);
  if (assignment.status !== 'pending') throw new AppError('Assignment is not pending', 400);

  const asset = await db('assets').where({ id: assignment.asset_id }).first();
  if (!asset) throw new AppError('Asset not found', 404);
  if (asset.status === 'retired' || asset.status === 'disposed') throw new AppError('Asset is not available', 400);

  await db.transaction(async (trx) => {
    await trx('assignments').where({ id }).update({
      status: 'assigned',
...
async function reject(id, rejectionReason, req) {
  ...
  await createNotification({
    ...
    message: `Your request for asset has been rejected.`,
    entityType: 'assignment',
    entityId: id,
  });
  ...
  return db('assignments')
    ...
    .orWhere(function () {
      this.where({ 'assignments.status': 'assigned' })
        ...
        .where('assignments.return_required', true);
    })
